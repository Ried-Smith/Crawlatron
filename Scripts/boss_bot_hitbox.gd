...
func _process(delta: float) -> void:
	if (fight_started): 
		...
		update_bars()
	if(tbHealth<=0):
		get_parent().get_parent().player.win()
...
func fight_ready():
	bot_name = "[center]CRAB_B0T SUPREME RULER[/center]"
...
func attack_1():
	...
	charge.start()
	battleInterface.enemy_block.attack_name.text = "[center]"+"Mega Crush"+"[/center]"
	dmg = 35
	type = PHYS
...
func attack_2():
	...
	charge.one_shot = true
	charge.start()
	battleInterface.enemy_block.attack_name.text = "[center]"+"Radiation Pulse"+"[/center]"
	dmg = 20
	type = FIRE
	battleInterface.enemy_block.atb.max_value = charge.wait_time
	#time_left = charge.time_left
func attack_3():
	...
	charge.one_shot = true
	charge.start()
	battleInterface.enemy_block.attack_name.text = "[center]"+"Seismic Wave"+"[/center]"
	dmg = 35
	type = ELEC
	battleInterface.enemy_block.atb.max_value = charge.wait_time
	#time_left = charge.time_left
func attack_4():
	...
	charge.one_shot = true
	charge.start()
	battleInterface.enemy_block.attack_name.text = "[center]"+"Omega Bott Slapper 9000"+"[/center]"
	dmg = 80
	type = FIRE
	battleInterface.enemy_block.atb.max_value = charge.wait_time
